...
for (let i=0;i<runners.length;i++){
    let runnerName=`${runners[i].first_name} ${runners[i].last_name}`;
...
let allCaps = [];
for (let i=0;i<runners.length;i++){
    let upperC=runners[i].first_name;
    allCaps.push(runners[i].first_name)
    
  }
  console.log(allCaps);
  allCaps.map(function toUpper(item){
...
ticketPriceTotal=runners.reduce((total, donationAmount)=>{
    return total+=donationAmount.donation;
  },0);
...
// Problem 1
// The bib printer wont print bibs in all caps, we need to return back to lower case. (.map)
let lowerCase=[];

for (let i=0;i<runners.length;i++){
  let lower=runners[i].first_name;
  lowerCase.push(runners[i].first_name)
  
}
console.log(lowerCase);
lowerCase.map(function lower(item){
  return item.toLowerCase();
});

// Problem 2
// we need to get all the even number runners to send a different start time (.filter)

evenId=runners.filter(number=>{
...
// Problem 3
// we need a list of each company and their donation amount to send a receipt for write off's. (.forEach)
let receipt=[];

  for (let i=0;i<runners.length;i++){
    let taxWriteOff=`${runners[i].company_name} amount: $${runners[i].donation}`;
    receipt.push(taxWriteOff)
  }

runners.forEach((cName,index)=>{
  console.log(receipt);
 });
